...
export default async function HrReportsPage({ searchParams }: { searchParams?: Promise<{ month?: string }> }) {
  ...
  return <div className="space-y-6"><section className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-[0.25em] text-indigo-600">HR Analytics</p><h1 className="mt-3 text-3xl font-black text-slate-950 dark:text-white">HR & Payroll Reports</h1><p className="mt-2 text-sm text-slate-500">Headcount, lifecycle, leave, corrections and payment status for {label(monthKey)}.</p></div><div className="flex flex-wrap gap-3"><Link href="/internal/hr" className="inline-flex h-11 items-center rounded-xl border border-slate-200 px-4 text-sm font-black dark:border-slate-700 dark:text-white">HR Center</Link><ReportExportButtons href={`/internal/hr/reports/export?month=${monthKey}`} compact /></div></div></section>
