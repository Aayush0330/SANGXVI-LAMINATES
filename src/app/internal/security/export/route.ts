...
export async function GET(request: NextRequest) {
  ...
  const columns = ["Time (IST)", "Event", "User Name", "User Email", "User Role", "Path", "IP Address", "User Agent", "Details"];
  const rows = filteredLogs.map((log) => [formatDateTime(log.createdAt), getEventLabel(log.eventType), log.userName || "Unknown", log.userEmail || "", log.userRole || "", log.path || "", log.ipAddress || "", log.userAgent || "", log.description || ""]);
  return createReportDownloadResponse(
    getReportFormat(request.nextUrl.searchParams.get("format")),
    {
      title: "Security Audit Logs",
      subtitle: `Event filter: ${eventType} | Search: ${query || "None"}`,
