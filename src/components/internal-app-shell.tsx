...
import { InternalMobileNavigation } from "@/components/internal-mobile-navigation";
...
export function InternalAppShell({
  ...
  const pageLabel = currentItem?.label ?? "Internal ERP";

  return (
    <div className="min-h-screen bg-[#f4f6f9] text-slate-950 dark:bg-slate-950 dark:text-white">
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-[280px] flex-col border-r border-slate-200 bg-white px-3 py-4 text-slate-950 shadow-[8px_0_32px_rgba(15,23,42,0.05)] lg:flex print:hidden dark:border-white/10 dark:bg-[#0b1220] dark:text-white dark:shadow-black/20">
        ...
        <div className="mt-5 flex min-h-0 flex-1 flex-col overflow-hidden">
          <InternalSidebarNav items={items} financeMode={financeMode} />
        </div>
      ...
      <div className="print:hidden">{assistant}</div>
